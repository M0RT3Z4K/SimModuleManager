( () => {
    ...
    const SPECS = {
        "MTProto": {
            "constructors": [{
                ...
                "id": -601545867,
                "predicate": "eitaa_token_updating",
                "params": [],
                "type": "EitaaTokenUpdating"
            }, {
                "id": -899560710,
        ...
        "API": {
            "constructors": [{
                ...
                "params": [{
                    ...
                    "name": "schedule_period_background_ms",
                    "type": "int"
                }, {
                    "name": "schedule_period_background_delay_ms",
                    "type": "int"
                }, {
                    "name": "upload_chunk_size_kb",
                    "type": "int"
                }, {
                    "name": "download_chunk_size_kb",
                    "type": "int"
                }],
                "type": "Config"
            }, {
                ...
                "params": [{
                    ...
                    "name": "new_secure_secret",
                    "type": "flags.2?bytes"
                }, {
                    "name": "new_secure_secret_id",
                    "type": "flags.2?long"
                }],
                "type": "account.PasswordInputSettings"
            }, {
                ...
                "params": [{
                    ...
                    "name": "thumbs",
                    "type": "flags.2?Vector<PhotoSize>"
                }],
                "type": "MessageMedia"
            }, {
                "id": -1263446626,
                "predicate": "liveStreamStateInit",
                "params": [],
                "type": "LiveStreamState"
            }, {
                "id": -1151640075,
                ...
                "id": -415214013,
                "predicate": "adsInputAdsLocationContacts",
                "params": [],
                "type": "AdsLocation"
            }, {
                "id": 1557811792,
                ...
                "id": 314488883,
                "predicate": "adsInputAdsLocationDialog",
                "params": [],
                "type": "AdsLocation"
            }, {
                "id": -1626620176,
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // SHARED DECODER — runs in both the main thread and inside the worker blob.
    // Everything between DECODER_START and DECODER_END is stringified and
    // injected verbatim into the worker via buildDecoderCode().
    // ══════════════════════════════════════════════════════════════════════════
    function buildDecoderCode(specsJSON) {
        return `
(function(self_or_window) {
"use strict";

var SPECS = ${specsJSON};

// ── BigInt long reader ──────────────────────────────────────────────────────
function readLong(low, high) {
  return (BigInt(high >>> 0) << 32n | BigInt(low >>> 0)).toString();
}
function uintToInt(val) { return val > 2147483647 ? val - 4294967296 : val; }
...
function TLDeserialization(buffer, options) {
  ...
  this.intView  = new Int32Array(this.buffer.buffer, this.buffer.byteOffset,
                                  Math.floor(this.buffer.byteLength / 4));
  this.byteView = this.buffer;
}
TLDeserialization.prototype.readInt = function(field) {
  if (this.offset + 4 > this.byteView.length)
    throw new Error("Overread " + field + " @" + this.offset);
  const i = this.intView[this.offset >> 2]; this.offset += 4; return i;
};
TLDeserialization.prototype.fetchInt    = function(f) { return this.readInt((f||"")+":int"); };
TLDeserialization.prototype.fetchDouble = function(f) {
  const b = new ArrayBuffer(8), iv = new Int32Array(b), dv = new Float64Array(b);
  iv[0] = this.readInt((f||"")+":dbl[lo]"); iv[1] = this.readInt((f||"")+":dbl[hi]");
  return dv[0];
};
TLDeserialization.prototype.fetchLong = function(f) {
  return readLong(this.readInt((f||"")+":lng[lo]"), this.readInt((f||"")+":lng[hi]"));
};
TLDeserialization.prototype.fetchBool = function(f) {
  const i = this.readInt((f||"")+":bool");
  if (i === 0x997275b5) return true;
  if (i === 0xbc799737) return false;
  this.offset -= 4; return this.fetchObject("Object", f);
};
TLDeserialization.prototype.fetchString = function(f) {
  let len = this.byteView[this.offset++];
  if (len === 254) {
    len = this.byteView[this.offset++] | (this.byteView[this.offset++] << 8) | (this.byteView[this.offset++] << 16);
  }
  let s = "";
  for (let i = 0; i < len; i++) s += String.fromCharCode(this.byteView[this.offset++]);
  while (this.offset % 4) this.offset++;
  try { return decodeURIComponent(escape(s)); } catch { return s; }
};
TLDeserialization.prototype.fetchBytes = function(f) {
  let len = this.byteView[this.offset++];
  if (len === 254) {
    len = this.byteView[this.offset++] | (this.byteView[this.offset++] << 8) | (this.byteView[this.offset++] << 16);
  }
  const b = this.byteView.slice(this.offset, this.offset + len);
  this.offset += len;
  while (this.offset % 4) this.offset++;
  return b;
};
TLDeserialization.prototype.fetchIntBytes = function(bits, typed, f) {
  const len = bits / 8;
  const r = typed ? this.byteView.slice(this.offset, this.offset + len)
                  : Array.from(this.byteView.slice(this.offset, this.offset + len));
  this.offset += len; return r;
};
TLDeserialization.prototype.fetchObject = function(type, field) {
  switch (type) {
    case "#": case "int": return this.fetchInt(field);
  ...
  if (type.substr(0,6) === "Vector" || type.substr(0,6) === "vector") {
    if (type.charAt(0) === "V") {
      const cid = uintToInt(this.readInt(field+"[id]"));
      if (cid === 0x3072cfa1) { this.fetchBytes(field+"[gz]"); return {__gzip:true}; }
      if (cid !== 0x1cb5c415) throw new Error("Bad vector cid: 0x"+(cid>>>0).toString(16));
    }
    const len = this.readInt(field+"[count]");
    const itemType = type.substr(7, type.length - 8);
    const out = [];
    for (let i = 0; i < len; i++) out.push(this.fetchObject(itemType, field+"["+i+"]"));
    return out;
  }
  const schemaMain = this.mtproto ? SPECS.MTProto : SPECS.API;
  const schemaFb   = this.mtproto ? SPECS.API : SPECS.MTProto;
  let cd = false;
  if (type.charAt(0) === "%") {
    const ct = type.substr(1);
    for (const c of schemaMain.constructors) { if (c.type === ct) { cd = c; break; } }
    if (!cd) for (const c of schemaFb.constructors) { if (c.type === ct) { cd = c; break; } }
    if (!cd) throw new Error("No constructor for type: " + type);
  } else if (type.charAt(0) >= "a" && type.charAt(0) <= "z") {
    for (const c of schemaMain.constructors) { if (c.predicate === type) { cd = c; break; } }
    if (!cd) for (const c of schemaFb.constructors) { if (c.predicate === type) { cd = c; break; } }
    if (!cd) throw new Error("No constructor for predicate: " + type);
  } else {
    const raw = this.readInt(field+"[id]");
    const cid = uintToInt(raw);
    if (cid === 0x3072cfa1) { this.fetchBytes(field+"[gz]"); return {__gzip:true}; }
    for (const c of schemaMain.constructors) { if (c.id === cid) { cd = c; break; } }
    if (!cd) for (const m of schemaMain.methods) { if (m.id === cid) { cd = m; break; } }
    if (!cd) for (const c of schemaFb.constructors) { if (c.id === cid) { cd = c; break; } }
    if (!cd) for (const m of schemaFb.methods) { if (m.id === cid) { cd = m; break; } }
    if (!cd) throw new Error("Unknown cid: 0x"+(raw>>>0).toString(16));
  }
  const predicate = cd.predicate || cd.method;
  const result = { _: predicate };
  const self = this;
  for (const param of cd.params) {
    let ptype = param.type;
    if (ptype === "#" && !result.pFlags) result.pFlags = {};
    let isCond = false;
    if (ptype.indexOf("?") !== -1) {
      isCond = true;
      const [cond, real] = ptype.split("?");
      const [flagField, bit] = cond.split(".");
      if (!(result[flagField] & (1 << +bit))) continue;
      ptype = real;
    }
    const value = self.fetchObject(ptype, field+"["+predicate+"]["+param.name+"]");
    if (isCond && ptype === "true") { result.pFlags = result.pFlags || {}; result.pFlags[param.name] = value; }
...
// ── Normalizer ──────────────────────────────────────────────────────────────
const MAX_D = 5, MAX_A = 30, MAX_S = 300, MAX_B = 32;
function normalize(v, d, seen) {
  if (d > MAX_D) return {__truncated:true};
  if (v == null) return v;
  if (typeof v === "string") return v.length > MAX_S ? v.slice(0, MAX_S) + "…" : v;
  if (typeof v === "number" || typeof v === "boolean") return v;
  if (v instanceof Uint8Array || v instanceof ArrayBuffer) {
    const b = v instanceof ArrayBuffer ? new Uint8Array(v) : v;
    return {__type:"bytes", len:b.length,
            hex:Array.from(b.slice(0,MAX_B)).map(x=>x.toString(16).padStart(2,"0")).join("")};
  }
  if (Array.isArray(v)) {
    const out = v.slice(0, MAX_A).map(x => normalize(x, d+1, seen));
    if (v.length > MAX_A) out.push({__truncated:true, total:v.length});
    return out;
  }
  if (typeof v === "object") {
    if (seen.has(v)) return {__circular:true};
    seen.add(v);
    const r = {};
    for (const k of Object.keys(v)) { try { r[k] = normalize(v[k], d+1, seen); } catch { r[k] = {__error:true}; } }
    return r;
  }
  return String(v);
}

// ── Unwrap wrapper chains ───────────────────────────────────────────────────
const WRAPPER_METHODS = new Set([
  "invokeWithLayer","invokeAfterMsg","invokeAfterMsgs",
  "initConnection","invokeWithoutUpdates","invokeWithTakeout","invokeWithMessagesRange"
]);
function unwrapMethod(obj, depth) {
  if (!obj || depth > 10) return obj;
  if (WRAPPER_METHODS.has(obj._) && obj.query && obj.query._) return unwrapMethod(obj.query, depth+1);
  return obj;
}

// ── Decode ──────────────────────────────────────────────────────────────────
const EITAA_OBJECT_CID = 2059302893;
function peekCid(bytes) {
  if (bytes.length < 4) return null;
  const view = new Int32Array(bytes.buffer, bytes.byteOffset, 1);
  return uintToInt(view[0]);
}
function tryDecode(bytes, mtproto) {
  try { return new TLDeserialization(bytes, {mtproto}).fetchObject("Object","root"); }
  catch(e) { return null; }
}
function decode(bytes) {
  const cid = peekCid(bytes);
  if (cid === EITAA_OBJECT_CID) {
    const outer = tryDecode(bytes, false);
    if (outer && outer.packed_data) {
      const pb = outer.packed_data instanceof Uint8Array ? outer.packed_data : new Uint8Array(outer.packed_data);
      const inner = tryDecode(pb, false) || tryDecode(pb, true);
      if (inner) {
        const u = unwrapMethod(inner, 0);
        return {
          method:  u._ || inner._ || "unknown",
          params:  normalize(u, 0, new WeakSet()),
          token:   outer.token  || null,
          imei:    outer.imei   || null,
          layer:   outer.layer  || null,
          wrapped: true
        };
      }
      return {
        method:"decode_error", error:"inner decode failed",
        params:{packed_data_len:pb.length,
                packed_first4:Array.from(pb.slice(0,4)).map(x=>x.toString(16)).join(" ")},
        token:outer.token||null, imei:outer.imei||null, layer:outer.layer||null
      };
    }
  }
  const direct = tryDecode(bytes, false) || tryDecode(bytes, true);
  if (direct) {
    const u = unwrapMethod(direct, 0);
    return { method:u._||direct._||"unknown", params:normalize(u,0,new WeakSet()),
             token:null, imei:null, layer:null, wrapped:false };
  }
  return {
    method:"decode_error",
    error:"both schemas failed, cid=0x"+(cid!=null?(cid>>>0).toString(16):"??"),
    params:{first8:Array.from(bytes.slice(0,8)).map(x=>x.toString(16).padStart(2,"0")).join(" ")}
  };
}

// ── Shared helpers ──────────────────────────────────────────────────────────
function isEitaa(url) { return url && (url.includes("eitaa.ir") || url.includes("eitaa.com")); }
...
self_or_window.__eitaaDecoder = { decode, isEitaa, toBytes };
})(typeof self !== "undefined" ? self : window);
`;
    }

    // ── Initialise decoder on main thread ────────────────────────────────────
    const decoderCode = buildDecoderCode(JSON.stringify(SPECS));
    // eslint-disable-next-line no-new-func
    new Function(decoderCode)();
    const {decode, isEitaa, toBytes} = window.__eitaaDecoder;

    // ── Post to page bus ──────────────────────────────────────────────────────
    function postToPage(payload) {
        window.postMessage({
            __eitaaInspector: true,
            type: "mtproto-call",
            payload
        }, "*");
    }

    function handle(url, bytes, direction) {
        if (!bytes || bytes.length < 4)
            return;
        const result = decode(bytes);
        if (result.method === "decode_error" && direction === "response")
            return;
        if (!result.method || result.method === "unknown")
            return;
        console.log("[EitaaInspector] " + direction + " " + result.method + " url=" + url);
        postToPage({
    ...
    // ── Worker constructor interceptor ────────────────────────────────────────
    // Injects the full decoder into the mtproto worker blob so it can do
    // complete deserialization including token, imei, layer and full params.
    function buildWorkerHookCode() {
        // Worker gets the full decoder code with SPECS baked in,
        // then a small fetch hook on top that uses it.
        const workerFetchHook = `
(function() {
  if (self.__eitaaWorkerHooked) return;
  self.__eitaaWorkerHooked = true;
  var _decode  = self.__eitaaDecoder.decode;
  var _isEitaa = self.__eitaaDecoder.isEitaa;
  var _toBytes = self.__eitaaDecoder.toBytes;
  ...
  self.fetch = function(input, init) {
    ...
    if (_isEitaa(url)) {
      try {
        var b = init && init.body ? _toBytes(init.body) : null;
        if (b && b.length >= 4) {
          var result = _decode(b);
          if (result.method && result.method !== "unknown" && result.method !== "decode_error") {
            self.postMessage({
              __eitaaWorker: true,
              method:   result.method,
              params:   result.params  || {},
  ...
  console.debug("[EitaaInspector] worker full-decoder fetch hook active");
})();
`;
        return decoderCode + "\n" + workerFetchHook;
    }

    if (!window.__eitaaWorkerHooked) {
        window.__eitaaWorkerHooked = true;
        const _Worker = window.Worker;
        const workerCode = buildWorkerHookCode();
        ...
        window.Worker = function(scriptURL, options) {
            ...
            // Only intercept the MTProto worker — rlottie and others use relative
            // importScripts() that break from a blob origin
            if (!resolvedURL.includes("mtproto")) {
                return new _Worker(scriptURL,options);
            }

            const blobCode = workerCode + "\nimportScripts(" + JSON.stringify(resolvedURL) + ");";
            const blob = new Blob([blobCode],{
                type: "application/javascript"
            });
            const blobURL = URL.createObjectURL(blob);
            const worker = new _Worker(blobURL,options);

            worker.addEventListener("message", function(e) {
                if (!e.data || !e.data.__eitaaWorker)
                    return;
                if (!e.data.method || e.data.method === "unknown")
                    return;
                postToPage({
                    ...
                    imei: e.data.imei || null,
                    layer: e.data.layer || null,
                    error: null,
                    wrapped: e.data.wrapped || false,
                    direction: e.data.direction || "request"
                });
            });
        ...
        console.debug("[EitaaInspector] Worker interceptor active (mtproto only)");
    }

    // ── Main-thread fetch hook ────────────────────────────────────────────────
    if (!window.__eitaaFetchHooked) {
        window.__eitaaFetchHooked = true;
        ...
        window.fetch = function(input, init) {
            ...
            if (isEitaa(url)) {
                try {
                    const b = (init && init.body) ? toBytes(init.body) : null;
                    if (b)
                        handle(url, b, "request");
                } catch (e) {}
                ...
                promise.then(function(response) {
                    try {
                        ...
                        clone.arrayBuffer().then(function(buf) {
                            if (buf.byteLength >= 4)
                                handle(url, new Uint8Array(buf), "response");
                        }).catch(function() {});
                    } catch (e) {}
                }).catch(function() {});
                return promise;
        ...
        ;
        console.debug("[EitaaInspector] fetch hook active, API methods: " + SPECS.API.methods.length);
    }

    // ── XHR hook (main thread) ────────────────────────────────────────────────
    if (!window.__eitaaXHRHooked) {
        window.__eitaaXHRHooked = true;
        ...
        XMLHttpRequest.prototype.send = function(body) {
            if (isEitaa(this.__eitaaUrl)) {
                const url = this.__eitaaUrl;
                try {
                ...
                this.addEventListener("load", function() {
                    try {
                        if (this.responseType === "arraybuffer" && this.response)
                            handle(url, new Uint8Array(this.response), "response");
                    } catch (e) {}
                });
            }
        ...
        ;
        console.debug("[EitaaInspector] XHR hook active");
    }
}
)();
